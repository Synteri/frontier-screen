...
Reads data/raw/*_activities.json, writes:
    data/clean/cnr2_clean.csv      one row per (molecule, best assay type)
    data/clean/cb1_clean.csv
...
def main():
    ...
    all_stats = {}
    for name, cid in [("CNR2", "CHEMBL253"), ("CB1", "CHEMBL218")]:
        print(f"[clean] {name}")
    ...
    both = set(cnr2["molecule_chembl_id"]) & set(cb1["molecule_chembl_id"])
    all_stats["overlap"] = {
        "molecules_with_both_targets": len(both),
        "cnr2_only": len(set(cnr2["molecule_chembl_id"]) - both),
        "cb1_only": len(set(cb1["molecule_chembl_id"]) - both),
    }
    print(f"[overlap] molecules with data on both targets: {len(both)}")

    lines = ["# Data quality report — CNR2 screen", "",
             "Generated by `scripts/02_clean_data.py`. Raw pulls cached in "
             "`data/raw/` (not committed); cleaned tables in `data/clean/`.",
             ""]
    for name in ["CNR2", "CB1"]:
        s = all_stats[name]
        lines += [
    ...
    lines += ["## Cross-target overlap", "",
              f"- Molecules with data on both CNR2 and CB1: **{o['molecules_with_both_targets']:,}**",
              f"- CNR2 only: {o['cnr2_only']:,}; CB1 only: {o['cb1_only']:,}",
              "",
    ...
    with open("data_quality_report.md", "w") as f:
        f.write("\n".join(lines) + "\n")
    print("[ok] data_quality_report.md")
    return 0
