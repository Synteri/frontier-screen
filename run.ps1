...
# The full ranking run happens after Jake reviews the pre-registration.
...
Invoke-Step "01 pull data"      "python scripts/01_pull_data.py"
Invoke-Step "02 clean data"     "python scripts/02_clean_data.py"
Invoke-Step "03 screen (test)"  "python scripts/03_screen.py --max-drugs 100"

Write-Host "`nPipeline logic verified on 100-drug subset." -ForegroundColor Green
Write-Host "Full ranking run (after Jake reviews preregistration/CNR2.md):" -ForegroundColor Yellow
Write-Host "  python scripts/03_screen.py --i-have-reviewed-the-prereg" -ForegroundColor Yellow
